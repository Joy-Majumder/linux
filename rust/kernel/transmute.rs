...
pub unsafe trait FromBytes {
        ...
        Self: Sized,
    {
        bytes
            ...
            .and_then(|(prefix, remainder)| Self::from_bytes(prefix).map(|s| (s, remainder)))
    }
        ...
        Self: AsBytes + Sized,
    {
        bytes
            ...
            .and_then(|(prefix, remainder)| Self::from_bytes_mut(prefix).map(|s| (s, remainder)))
    }
        ...
        Self: Sized,
    {
        bytes
            .split_at_checked(size_of::<Self>())
            .and_then(|(prefix, remainder)| Self::from_bytes_copy(prefix).map(|s| (s, remainder)))
    }
}
